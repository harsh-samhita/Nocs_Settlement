...
transaction_id_1 = f"tc15-txn-{unique_id}"
transaction_id_2 = f"tc15-txn-{unique_id}-2"
message_id_1 = f"tc15-msg-{unique_id}-1"
message_id_2 = f"tc15-msg-{unique_id}-2"
current_timestamp = datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z'

def test_collector_settlement():
    print("TC_15: 2-way Reconciliation - Collector NIL Settle")
    print("Step 1: Collector sends NIL settlement type")
    collector_payload = {
        "context": {
            "domain": "ONDC:NTS10",
    ...
    try:
        ...
        if response.status_code == 200:
            print("\n✅ Collector NIL settlement got ACK!")
            return True, transaction_id_1, message_id_2
        else:
            print("\n❌ Collector NIL settlement got NACK!")
...
def test_receiver_settlement(transaction_id_2, message_id_2):
    ...
    receiver_payload = {
        ...
        "message": {
            ...
            "settlement": {
                "type": "NP-NP",
                "id": f"settlement-tc15-receiver-{transaction_id_2}",
                "orders": [
                    {
                        "id": f"order-tc15-{transaction_id_2}",
                        "inter_participant": {
                            "amount": {
                                "currency": "INR",
                                "value": "1000.00"
                        ...
                        "provider": {
                            "id": f"prvdr-tc15-{transaction_id_2}",
                            "name": "Test Provider",
                            "bank_details": {
...
def test_nocs_api():
    ...
    if collector_success:
        ...
        if receiver_success:
            print(f"\n🎉 TC_15 Complete: Both settlements got ACK successful!")
        else:
            print("\n❌ TC_15 Failed: Receiver settlement got NACK!")
    else:
        print("\n❌ TC_15 Failed: Collector settlement got NACK!")
