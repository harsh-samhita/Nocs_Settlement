...
transaction_id_1 = f"tc11-txn-{unique_id}"
transaction_id_2 = f"tc11-txn-{unique_id}-2"
message_id_1 = f"tc11-msg-{unique_id}-1"
message_id_2 = f"tc11-msg-{unique_id}-2"
current_timestamp = datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z'

def test_collector_settlement():
    print("TC_11: 2-way Reconciliation - Integer Mismatch in Collector")
    print("Step 1: Collector sends valid amount")
    
    # Collector settlement payload - Valid amounts
    collector_payload = {
        "context": {
            "domain": "ONDC:NTS10",
        ...
        "message": {
            ...
            "settlement": {
                "type": "NP-NP",
                "id": f"settlement-tc11-{transaction_id_1}",
                "orders": [
                    {
                        "id": f"order-tc11-{unique_id}",
                        "inter_participant": {
                            "amount": {
                                "currency": "INR",
                                "value": "1000.00"  # Valid amount
                            }
                        },
                        "collector": {
                            "amount": {
                                "currency": "INR",
                                "value": "50.00"  # Valid amount
                            }
                        },
                        "provider": {
                            "id": f"prvdr-tc11-{unique_id}",
                            "name": "Test Provider",
                            "bank_details": {
...
def test_receiver_settlement(transaction_id_2, message_id_2):
    print("Second: Receiver sends mis-matched integer amount in Collector")
    ...
    receiver_payload = {
        ...
        "message": {
            ...
            "settlement": {
                "type": "NP-NP",
                "id": f"settlement-tc11-receiver-{transaction_id_2}",
                "orders": [
                    {
                        "id": f"order-tc11-{unique_id}",
                        "inter_participant": {
                            "amount": {
                                "currency": "INR",
                                "value": "1000.00"  # Same as Interparticipant
                            }
                        },
                        "collector": {
                            "amount": {
                                "currency": "INR",
                                "value": "60"  # Mis-matched integer amount
                            }
                        },
                        "provider": {
                            "id": f"prvdr-tc11-{transaction_id_2}",
                            "name": "Test Provider",
                            "bank_details": {
                                "account_no": "1234567890",
                                "ifsc_code": "IFSC0001"
...
def test_nocs_api():
    ...
    if collector_success:
        ...
        if receiver_success:
            print(f"\n🎉 TC_11 Complete: Both settlements got ACK successful!")
        else:
            print("\n❌ TC_11 Failed: Receiver settlement got NACK!")
    else:
        print("\n❌ TC_11 Failed: Collector settlement got NACK!")
