...
def test_collector_settlement():
    """First: Collector sends valid data for ISN receiver"""
    print("=== TC_17: 2-way Reconciliation for ISN receiver (ISN without NOCA account) ===")
    ...
    transaction_id = f"tc17-txn-{unique_id}"
    message_id_1 = f"tc17-msg-{unique_id}-1"
    message_id_2 = f"tc17-msg-{unique_id}-2"
    ...
    collector_payload = {
        ...
        "message": {
            ...
            "settlement": {
                "type": "NP-NP",
                "id": f"settlement-tc17-{unique_id}",
                "orders": [
                    {
                        "id": f"order-tc17-{unique_id}",
                        "inter_participant": {
                            "amount": {
                                "currency": "INR",
                                "value": "1000.00"
...
def test_receiver_settlement(transaction_id, message_id_2):
    """Second: Receiver (ISN) sends valid data"""
    print("\nStep 2: Receiver (ISN) sends /settle payload with valid data")
    print("Expected: ACK, Successful order settlement with settlement reference number")
    ...
    receiver_payload = {
        "context": {
            ...
            "version": "2.0.0", 
            "action": "settle",
            "bap_id": "staging-smartsell.samhita.org",  # Same BAP ID for signature validation
            "bap_uri": "https://staging-smartsell.samhita.org/ondc/api/v1",
            "bpp_id": "sa_nocs.nbbl.com",
            "bpp_uri": "https://sa_nocs.nbbl.com/nocs_test",
        ...
        "message": {
            ...
            "settlement": {
                "type": "NP-NP",
                "id": f"settlement-tc17-receiver-{transaction_id}",
                "orders": [
                    {
                        "id": f"order-tc17-{transaction_id}",
                        "inter_participant": {
                            "amount": {
                                "currency": "INR",
                        ...
                        "provider": {
                            "id": f"prvdr-tc17-{transaction_id}",
                            "name": "Test Provider",
                            "bank_details": {
                                "account_no": "1234567890",
                                "ifsc_code": "IFSC0001"
    ...
    try:
        ...
        if response.status_code == 200:
            print("\n✅ Receiver (ISN) settlement successful!")
            return True
        else:
            print("\n❌ Receiver (ISN) settlement failed!")
            return False
...
def test_nocs_api():
    """Run the complete 2-way reconciliation test with ISN receiver"""
    ...
    if collector_success:
        ...
        if receiver_success:
            print(f"\n🎉 TC_17 Complete: Both settlements successful!")
            print(f"Transaction ID: {transaction_id}")
            print("ISN receiver test completed!")
            print("Check database for settlement reference number!")
        else:
            print("\n❌ TC_17 Failed: Receiver settlement failed!")
    else:
        print("\n❌ TC_17 Failed: Collector settlement failed!")
