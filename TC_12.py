...
transaction_id_1 = f"tc12-txn-{unique_id}"
transaction_id_2 = f"tc12-txn-{unique_id}-2"
message_id_1 = f"tc12-msg-{unique_id}-1"
message_id_2 = f"tc12-msg-{unique_id}-2"
current_timestamp = datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z'
def test_collector_settlement():
    print("TC_12: 2-way Reconciliation - Mis-matched orderId")
    print("Step 1: Collector sends two order details with valid data")
    
    # Collector settlement payload - Two orders with valid data
    collector_payload = {
        "context": {
            "domain": "ONDC:NTS10",
        ...
        "message": {
            ...
            "settlement": {
                "type": "NP-NP",
                "id": f"settlement-tc12-collector-{transaction_id_1}",
                "orders": [
                    {
                        "id": f"order-tc12-{unique_id}-1",  # First order
                        "inter_participant": {
                            "amount": {
                                "currency": "INR",
                                "value": "1000.00"
                    ...
                    {
                        "id": f"order-tc12-{unique_id}-2",  # Second order
                        "inter_participant": {
                            "amount": {
                                "currency": "INR",
                                "value": "500.00"
...
def test_receiver_settlement(transaction_id_2, message_id_2):
    print("Second: Receiver sends different orderId for one of the orders")
    ...
    receiver_payload = {
        "context": {
            ...
            "bpp_id": BPP_ID,
            "bpp_uri": BPP_URI,
            "transaction_id": transaction_id_2,  # Same transaction_id
        ...
        "message": {
            ...
            "settlement": {
                "type": "NP-NP",
                "id": f"settlement-tc12-receiver-{transaction_id_2}",
                "orders": [
                    {
                        "id": f"order-tc12-{unique_id}-1",  # Same order ID
                        "inter_participant": {
                            "amount": {
                                "currency": "INR",
                        ...
                        "provider": {
                            "id": f"prvdr-tc12-{transaction_id_2}-1",
                            "name": "Test Provider 1",
                            "bank_details": {
                                "account_no": "1234567890",
                                "ifsc_code": "IFSC0001"
                    ...
                    {
                        "id": f"order-tc12-{unique_id}-2-DIFFERENT",  # Different order ID!
                        "inter_participant": {
                            "amount": {
                                "currency": "INR",
                                "value": "500.00"  # Same as collector
                            }
                        },
                        "collector": {
                            "amount": {
                                "currency": "INR",
                                "value": "25.00"  # Same as collector
                            }
                        },
                        "provider": {
                            "id": f"prvdr-tc12-{transaction_id_2}-2",
                            "name": "Test Provider 2",
                            "bank_details": {
                                "account_no": "0987654321",
                                "ifsc_code": "IFSC0002"
                            ...
                            "amount": {
                                "currency": "INR",
                                "value": "400.00"  # Same as collector
                            }
                        },
                        "self": {
                            "amount": {
                                "currency": "INR",
                                "value": "100.00"  # Same as collector
                            }
                        }
                    }
                ]
...
def test_nocs_api():
    ...
    if collector_success:
        ...
        if receiver_success:
            print(f"\n🎉 TC_12 Complete: Both settlements got ACK successful!")
        else:
            print("\n❌ TC_12 Failed: Receiver settlement got NACK!")
    else:
        print("\n❌ TC_12 Failed: Collector settlement got NACK!")
