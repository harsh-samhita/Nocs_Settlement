...
transaction_id_1 = f"tc10-txn-{unique_id}"
transaction_id_2 = f"tc10-txn-{unique_id}-2"
message_id_1 = f"tc10-msg-{unique_id}-1"
message_id_2 = f"tc10-msg-{unique_id}-2"
...
def test_collector_settlement():
    ...
    collector_payload = {
        ...
        "message": {
            ...
            "settlement": {
                "type": "NP-NP",
                "id": f"settlement-tc10-{transaction_id_1}",
                "orders": [
                    {
                        "id": f"order-tc10-{unique_id}",
                        "inter_participant": {
                            "amount": {
                                "currency": "INR",
                                "value": "1000.00"
...
def test_receiver_settlement(transaction_id_2, message_id_2):
    print("Second: Receiver sends mis-matched integer amount in Interparticipant")
    ...
    receiver_payload = {
        ...
        "message": {
            ...
            "settlement": {
                "type": "NP-NP",
                "id": f"settlement-tc10-receiver-{transaction_id_2}",
                "orders": [
                    {
                        "id": f"order-tc10-{unique_id}",
                        "inter_participant": {
                            "amount": {
                                "currency": "INR",
                                "value": "1200"  # Mis-matched integer amount
                            }
                        },
                        "collector": {
                            "amount": {
                        ...
                        "provider": {
                            "id": f"prvdr-tc10-{transaction_id_2}",
                            "name": "Test Provider",
                            "bank_details": {
                                "account_no": "1234567890",
                                "ifsc_code": "IFSC0001"
...
def test_nocs_api():
    print("TC_10: 2-way Reconciliation - Integer Mismatch in Interparticipant")
    ...
    if collector_success:
        ...
        if receiver_success:
            print(f"\n🎉 TC_10 Complete: Both settlements got ACK successful!")
        else:
            print("\n❌ TC_10 Failed: Receiver settlement got NACK!")
    else:
        print("\n❌ TC_10 Failed: Collector settlement got NACK!")
