...
transaction_id_1 = f"tc05-txn-{unique_id}-1"
transaction_id_2 = f"tc05-txn-{unique_id}-2"
message_id = f"tc05-msg-{unique_id}"  # SAME message_id for both requests
current_timestamp = datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z'

def test_nocs_api():
    print("TC_05: Duplicate message_id")
    ...
    try:
        ...
        print(f"Status: {response.status_code} | Txn: {transaction_id_1} | Msg: {message_id} | timestamp: {payload1['context']['timestamp']}")
        print(f"Response: {json.dumps(response.json(), separators=(',', ':'))}")
        
        time.sleep(2)
        print("\n--- Waiting 2 seconds ---\n")
        
        # Second request with SAME message_id but different transaction_id
        payload2 = {
            "context": {
                "domain": "ONDC:NTS10",
                "location": {"country": {"code": "IND"}, "city": {"code": "*"}},
            ...
            "message": {
                ...
                "settlement": {
                    "type": "NP-NP",
                    "id": f"settlement-tc05-{unique_id}-2",
                    "orders": [
                        {
                            "id": f"order-tc05-{unique_id}-2",
                            "inter_participant": {"amount": {"currency": "INR", "value": "1000.00"}},
                            "collector": {"amount": {"currency": "INR", "value": "50.00"}},
                            "provider": {
                                "id": f"prvdr-tc05-{unique_id}-2",
                                "name": "Test Provider",
                                "bank_details": {"account_no": "1234567890", "ifsc_code": "IFSC0001"},
                                "amount": {"currency": "INR", "value": "800.00"}
                            },
        ...
        print(f"Status: {response2.status_code} | Txn: {transaction_id_2} | Msg: {message_id} | timestamp: {payload2['context']['timestamp']}")
