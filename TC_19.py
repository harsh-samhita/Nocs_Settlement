...
from creds import SETTLE_ENDPOINT, get_headers, COLLECTOR_APP_ID, RECEIVER_APP_ID, DIFFERENT_COLLECTOR_APP_ID, BAP_ID, BAP_URI, BPP_ID, BPP_URI,PRIVATE_KEY,BUYER_KEY

unique_id = str(uuid.uuid4())[:8]
transaction_id_1 = f"tc19-txn-{unique_id}"
transaction_id_2 = f"tc19-txn-{unique_id}-2"
message_id_1 = f"tc19-msg-{unique_id}-1"
message_id_2 = f"tc19-msg-{unique_id}-2"
current_timestamp = datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z'

def test_collector_settlement():
    print("TC_19: 2-way Reconciliation - Collector Settle Fails")
    print("Step 1: Collector sends /settle payload with invalid data")
    collector_payload = {
        "context": {
            "domain": "ONDC:NTS10",
        ...
        "message": {
            "collector_app_id": DIFFERENT_COLLECTOR_APP_ID,  # MISMATCH with bap_id!
            "receiver_app_id": RECEIVER_APP_ID,
            "settlement": {
                "type": "NP-NP",
                "id": f"settlement-tc19-{transaction_id_1}",
                "orders": [
                    {
                        "id": f"order-tc19-{transaction_id_1}",
                        "inter_participant": {
                            "amount": {
                                "currency": "INR",
                                "value": "1000.00"
                        ...
                        "provider": {
                            "id": f"prvdr-tc19-{transaction_id_1}",
                            "name": "Test Provider",
                            "bank_details": {
...
def test_receiver_settlement(transaction_id_2, message_id_2):
    print("Second: Receiver sends valid data")
    receiver_payload = {
        "context": {    
            "domain": "ONDC:NTS10",
            "location": {"country": {"code": "IND"}, "city": {"code": "*"}},
            "version": "2.0.0", 
            "action": "settle",
            "bap_id": RECEIVER_APP_ID,  # Same BAP for signature
            "bap_uri": BAP_URI,
            "bpp_id": BPP_ID,
            "bpp_uri": BPP_URI,
        ...
        "message": {
            ...
            "settlement": {
                "type": "NP-NP",
                "id": f"settlement-tc19-receiver-{transaction_id_2}",
                "orders": [
                    {
                        "id": f"order-tc19-{transaction_id_2}",
                        "inter_participant": {
                            "amount": {
                                "currency": "INR",
                        ...
                        "provider": {
                            "id": f"prvdr-tc19-{transaction_id_2}",
                            "name": "Test Provider",
                            "bank_details": {
                                "account_no": "1234567890",
                                "ifsc_code": "IFSC0001"
...
def test_nocs_api():
    ...
    if collector_success:
        ...
        if receiver_success:
            print(f"\n🎉 TC_19 Complete: Both settlements got ACK successful!")
        else:
            print("\n❌ TC_19 Failed: Receiver settlement got NACK!")
    else:
        print("\n❌ TC_19 Failed: Collector settlement got NACK!")
