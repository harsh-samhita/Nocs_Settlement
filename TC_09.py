...
transaction_id_1 = f"tc09-txn-{unique_id}"
transaction_id_2 = f"tc09-txn-{unique_id}-2"
message_id_1 = f"tc09-msg-{unique_id}-1"
message_id_2 = f"tc09-msg-{unique_id}-2"
current_timestamp = datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z'

def test_collector_settlement():
    """First: Collector sends valid amount with decimals"""
    print("TC_09: 2-way Reconciliation - Decimal Mismatch")
    print("Step 1: Collector sends valid amount with decimals")
    
    # Collector settlement payload with decimal amounts
    collector_payload = {
        "context": {
            "domain": "ONDC:NTS10",
        ...
        "message": {
            ...
            "settlement": {
                "type": "NP-NP",
                "id": f"settlement-tc09-{transaction_id_1}",
                "orders": [
                    {
                        "id": f"order-tc09-{unique_id}",
                        "inter_participant": {
                            "amount": {
                                "currency": "INR",
                                "value": "1000.50"  # Decimal amount
                            }
                        },
                        "collector": {
                            "amount": {
                                "currency": "INR",
                                "value": "50.25"  # Decimal amount
                            }
                        },
                        "provider": {
                            "id": f"prvdr-tc09-{unique_id}",
                            "name": "Test Provider",
                            "bank_details": {
                            ...
                            "amount": {
                                "currency": "INR",
                                "value": "800.40"  # Decimal amount
                            }
                        },
                        "self": {
                            "amount": {
                                "currency": "INR",
                                "value": "200.10"  # Decimal amount
                            }
                        }
                    }
                ]
...
def test_receiver_settlement(transaction_id_2, message_id_2):
    print("Second: Receiver sends matching integer but decimal mismatch")
    
    # Receiver settlement payload (integer amounts, decimal mismatch)
    receiver_payload = {
        "context": {
            "domain": "ONDC:NTS10",
            "location": {"country": {"code": "IND"}, "city": {"code": "*"}},
        ...
        "message": {
            ...
            "settlement": {
                "type": "NP-NP",
                "id": f"settlement-tc09-receiver-{transaction_id_2}",
                "orders": [
                    {
                        "id": f"order-tc09-{unique_id}",
                        "inter_participant": {
                            "amount": {
                                "currency": "INR",
                                "value": "1000"  # Integer - decimal mismatch
                            }
                        },
                        "collector": {
                            "amount": {
                                "currency": "INR",
                                "value": "50"  # Integer - decimal mismatch
                            }
                        },
                        "provider": {
                            "id": f"prvdr-tc09-{transaction_id_2}",
                            "name": "Test Provider",
                            "bank_details": {
                                "account_no": "1234567890",
                                "ifsc_code": "IFSC0001"
                            ...
                            "amount": {
                                "currency": "INR",
                                "value": "800"  # Integer - decimal mismatch
                            }
                        },
                        "self": {
                            "amount": {
                                "currency": "INR",
                                "value": "200"  # Integer - decimal mismatch
                            }
                        }
                    }
                ]
...
def test_nocs_api():
    ...
    if collector_success:
        ...
        if receiver_success:
            print(f"\n🎉 TC_09 Complete: Both settlements got ACK successful!")
        else:
            print("\n❌ TC_09 Failed: Receiver settlement got NACK!")
    else:
        print("\n❌ TC_09 Failed: Collector settlement got NACK!")
